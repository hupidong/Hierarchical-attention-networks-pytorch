...
class WordAttNet(nn.Module):
    def __init__(self, word2vec_path, hidden_size=50):
        ...
        word_dict = torch.from_numpy(np.concatenate([unknown_word, word_dict], axis=0).astype(np.float))

        self.word_weight = nn.Parameter(torch.Tensor(2 * hidden_size, 2 * hidden_size))
        self.word_bias = nn.Parameter(torch.Tensor(1, 2 * hidden_size))
        self.context_weight = nn.Parameter(torch.Tensor(2 * hidden_size, 1))

        self.lookup = nn.Embedding(num_embeddings=dict_len, embedding_dim=embed_size).from_pretrained(word_dict)
